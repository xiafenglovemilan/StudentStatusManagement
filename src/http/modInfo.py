...
@routes.post('/modify/password')
async def mod_password(request):
    session = await get_session(request)
    session_name = session['userId'] if 'userId' in session else None
    data = await request.json()
    data['userId'] = session_name
    if session_name:
        sel_obj = update.UpdateData(data)
        is_success = sel_obj.update_password()
        if is_success:
            result = {
                "code": 200,
                "msg": "OK",
...
@routes.post('/modify/student_grade')
async def mod_student_grade(request):
    session = await get_session(request)
    session_name = session['userId'] if 'userId' in session else None
    data = await request.json()
    if session_name:
        sel_obj = update.UpdateData(params=data)
        is_success = sel_obj.update_student_grade()
        if is_success:
            result = {
                "code": 200,
                "msg": "OK",
